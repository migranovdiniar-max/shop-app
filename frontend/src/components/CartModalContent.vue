<template>
  ...
  <div v-if="items.length === 0" class="empty">
    ...
    <div class="emptySub">Добавь товары из каталога</div>
  </div>
  ...
  <div v-else class="body">
    ...
    <div class="list">
      <div v-for="p in items" :key="p.id" class="row">
        ...
        <div class="mid">
          ...
          <div class="qty">
            <button class="qtyBtn" type="button" @click="$emit('set-qty', p.id, p.qty - 1)">−</button>
    ...
    <div class="footer">
      ...
      <button class="btnGreen" type="button" @click="$emit('checkout')">Оформить</button>
    </div>
  </div>
</template>
...
const emit = defineEmits(["open-product", "set-qty", "remove", "remove-many", "checkout"]);
